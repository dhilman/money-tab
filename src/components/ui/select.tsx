...
function useIsOpenDebounced() {
  const [isOpen, setIsOpen] = React.useState(false);
  const lastOpenTime = React.useRef(0);

  // Wrap the setIsOpen to prevent the dropdown from closing if it was opened
  // in the last 200ms.
  // Needed as on mobile, dropdown closes when input field is unfocused.
  const setIsOpenWrapper = React.useCallback(
    (value: boolean) => {
      if (value) {
        ...
        const now = Date.now();
        if (now - lastOpenTime.current < 200) {
          return;
        }
      }
      setIsOpen(value);
